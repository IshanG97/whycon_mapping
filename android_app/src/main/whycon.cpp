//THIS IS A MODIFIED VERSION OF WHYCON IMPLEMENTED TO RUN THE MAPPING SYSTEM.
//MODIFIED BY ISHAN GODAWATTA, STUDENT ID 9651584, MAY 2020, THE UNIVERSITY OF MANCHESTER.
//ORIGINAL LOCATION: whycon-orig/src/main
...
#include <stdio.h> //EDITED. GIVES ACCESS TO FILE MANIPULATION


//-----These parameters need to be adjusted by the user -----------------------

//Adjust camera resolution here
int  imageWidth= 640;	//EDITED
int  imageHeight = 480;	//EDITED
...
bool saveVideo = false;		//save video to output folder? EDITED. DISABLED TO IMPROVE PROCESSING SPEED
...
bool initializeLogging()
{
	...
	//EDITED FROM HERE. TO GET AN APPROPRIATE OUTPUT FOR THE MAPPING SYSTEM TO USE.
		//char timeStr[100];
		//time_t timeNow;
		//time(&timeNow);
	...
	//EDITED FROM HERE. SAVE REASON AS THE ABOVE EDITS
	int gridnum;
	FILE *o;
	o=fopen("/home/pi/files/gridnum.txt","r");
	if((o=fopen("/home/pi/files/gridnum.txt","r"))!=NULL)
	{
		fscanf(o,"%i",&gridnum);
		fclose(o);
	}
	
	sprintf(logFileName,"/home/pi/files/whycon_log_%i.txt",gridnum);
	
	
	robotPositionLog = fopen(logFileName,"w");
	if (robotPositionLog == NULL)
	{
		fprintf(stderr,"Cannot write to log file %s. Does the directory exist?\n",logFileName);
		return false;
	}
	return true;
	//EDITED TO HERE
